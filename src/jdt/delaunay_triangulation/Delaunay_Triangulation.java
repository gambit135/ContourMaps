package jdt.delaunay_triangulation;
 ...
 * is 3D surface (terrain) presentation. <br> The class main properties are the
 * following:<br> - fast point location. (O(n^0.5)), practical runtime is often
 * very fast. <br> - handles degenerate cases and none general position input
 * (ignores duplicate points). <br> - save & load from\to text file in TSIN
 * format. <br> - 3D support: including z value approximation. <br> - standard
 * java (1.5 generic) iterators for the vertices and triangles. <br> - smart
 * iterator to only the updated triangles - for terrain simplification <br> <br>
 *
 ...
 *
 * @author Boaz Ben Moshe 5/11/05 <br> The project uses some ideas presented in
 * the VoroGuide project, written by Klasse f?r Kreise (1996-1997), For the
 * original applet see: http://www.pi6.fernuni-hagen.de/GeomLab/VoroGlide/ .
 * <br>
 */
public class Delaunay_Triangulation {
     ...
     * it as an smf file as try to read it as .tsin <br> Note: duplicated points
     * are ignored! <br> SMF file has an OFF like format (a face (f) is
     * presented by the indexes of its points - starting from 1 - not from 0):
     * <br> begin <br> v x1 y1 z1 <br> ... <br> v xn yn zn <br> f i11 i12 i13
     * <br> ... <br> f im1 im2 im3 <br> end <br> <br> The tsin text file has the
     * following (very simple) format <br> vertices# (n) <br> x1 y1 z1 <br> ...
     * <br> xn yn zn <br>
     *
     *
     */
    ...
    /**
     * @return the number of triangles in the triangulation. <br /> Note:
     *         includes infinife faces!!.
     */
     ...
     * following format <br> #vertices (n) <br> x1 y1 z1 <br> ... <br> xn yn zn
     * <br>
     */
    ...
    public void write_smf(String smfFile) throws Exception {
        ...
        //Drops vertices on an array
        for (int i = 0; i < len; i++) {
            ans[i] = it.next();
        }
        //Sorts them on x-ascending order.
        Arrays.sort(ans, comp);
        
        FileWriter fw = new FileWriter(smfFile);
        PrintWriter os = new PrintWriter(fw);
        
        // prints the smf file header:
        os.println("begin");

        //Writes each vertex, denoted with the v letter
        for (int i = 0; i < len; i++) {
            os.println("v " + ans[i].toFile());
        }        
        int t = 0, i1 = -1, i2 = -1, i3 = -1;        
        //For all triangles stored in the current triangulation 
        for (Iterator<Triangle_dt> dt = this.trianglesIterator(); dt.hasNext();) {
            Triangle_dt curr = dt.next();
            t++;
            if (!curr.halfplane) {  //Si no es un medio plano
    ...
    /**
     * compute the number of vertices in the convex hull. <br /> NOTE: has a
     * 'bug-like' behavor: <br /> in cases of colinear - not on a asix parallel
     * rectangle, colinear points are reported
     *
    ...
    public static Point_dt[] read_tsin(String tsinFile) throws Exception {
        ...
        for (int i = 0; i < numOfVer; i++) {
            ...
            //Restáurame si todo se va al diablo
            //ans[i] = new Point_dt((int) d1, (int) d2, d3);
            ans[i] = new Point_dt((int) d1, (int) d2, d3, color);
        }
        return ans;
    }
